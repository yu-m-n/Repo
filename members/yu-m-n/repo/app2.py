...
accuracy, f1, precision, recall = calculate_metrics(val_df)
high_risk_ratio = (val_df["pred_prob"] >= 0.7).mean()
avg_pred_prob = val_df["pred_prob"].mean()
...
# 재계산
f_accuracy, f_f1, f_precision, f_recall = calculate_metrics(filtered_df) if len(filtered_df) > 0 else (0, 0, 0, 0)
...
with r3:
    risk_past = filtered_df.groupby("risk_level", observed=False)["past_return_rate"].mean().reindex(["Low", "Medium", "High"])
    fig, ax = plt.subplots(figsize=(5, 4))
